    ...
    <div>
        ...
        <div class="card w-full mh-100">
            <div class="card-body" v-if="visible">
                ...
                <profile-form :errors="errors" :data="data" @update="update">
                    <template v-slot:buttons>
                        <button class="btn btn-primary" @click="save" type="submit">Salvar</button>
                        <button class="btn btn-danger" @click="goTo('/profiles')" type="submit">Cancelar</button>
                    </template>
                </profile-form>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            getCompany(uuid) {
                axios.get('/api/v1/profiles/' + uuid).then(response => {
                    this.data = response.data.data;
                    this.subtitle = this.data.name || 'Novo perfil';
                }).then(() => {
                    this.visible = true;
                })
            },
            ...
            save() {
                ...
                axios.put('/api/v1/profiles/' + this.uuid, this.data).then(response => {
