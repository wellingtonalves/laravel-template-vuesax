        ...
        <div class="card w-full mh-100">
            <div class="card-body" v-if="visible">
                <div class="d-flex flex-row w-full pb-2">
                    ...
                    <span class="ml-auto">
                        <button v-if="permission('clients-store')" @click="$router.push('/clients/create')"
                        ...
                        <table class="table">
                            ...
                            <tr v-for="item in data" :key="item.id">
                                ...
                                <td class="text-right">
                                    ...
                                    <button v-if="permission('clients-edit')" @click="editUser(item.uuid)" type="button"
                                            class="btn btn-primary btn-sm">Editar
                                    </button>
                                    <button v-if="permission('clients-destroy')" @click="deleteItem(item)" type="button"
...
<script>
    export default {
        ...
        methods: {
            getAll(filtro = '') {
                axios.get('/api/v1/clients' + filtro).then(response => {
            ...
            editUser(uuid) {
                this.$router.push(`/clients/${uuid}/edit`);
            },
            sendDelete(uuid) {
                this.modalDelete = false;
                axios.delete('/api/v1/clients/' + uuid).then(response => {
